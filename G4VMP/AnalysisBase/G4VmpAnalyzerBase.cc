...
#include "G4VMP/AnalysisBase/G4VmpAnalyzerBase.hh"
...
// G4 XSec 
// --> obsolete as of 10.7.rXX cycle <-- #include "Geant4/G4HadronCrossSections.hh"
...
//
// --> obsolete --> #include "art/Framework/Services/Optional/TFileService.h"
#include "art_root_io/TFileService.h"
//
// Run/Eevent data products
#include "G4VMP/DataProducts/Interactions/G4VmpVtx.hh"        // Event data product
// Incoming Event data products
#include "G4VMP/DataProducts/EventGenerators/GenParticle.hh"
#include "G4VMP/DataProducts/EventGenerators/GenParticleCollection.hh"
...
G4VMP::G4VmpAnalyzerBase::G4VmpAnalyzerBase( const fhicl::ParameterSet& p )
  : G4VMP::AnalyzerWithExpDataBase(p), // art::EDAnalyzer(p),
...
G4VMP::G4VmpAnalyzerBase::~G4VmpAnalyzerBase()
{
...
void G4VMP::G4VmpAnalyzerBase::beginRun( const art::Run& r )
{
...
//
   ...
   art::Handle<G4VmpModelConfig> physcfg;
...
void G4VMP::G4VmpAnalyzerBase::endJob()
{
...
//   BeamThinTargetConfig* tconf =
   tfs->makeAndRegister<G4VmpBeamThinTargetConfig>( "Beam_ThinTarget_Config", 
                                                    "Beam-Momentum-TargetNucleus",
                                                    fBTConf );
...
void G4VMP::G4VmpAnalyzerBase::initXSecOnTarget( const std::string& mname,
                                                 const G4VMP::G4VmpParticle& part )
{
...
/* --> obsolete as of 10.7.rXX cycle <--
   fXSecOnTarget = (G4HadronCrossSections::Instance())->GetInelasticCrossSection( &g4pdyn, Z, A );
*/

   G4VCrossSectionDataSet* g4xs = 0;
   
   if ( g4pd == G4Proton::Definition() || g4pd == G4Neutron::Definition() ) 
   {
      g4xs = new G4BGGNucleonInelasticXS( g4pd );
   } 
   else if ( g4pd == G4PionPlus::Definition() || g4pd == G4PionMinus::Definition() ) 
   {
      g4xs = new G4BGGPionInelasticXS( g4pd );
   } 
   else if ( g4pd->GetBaryonNumber() > 1 ) 
   {  // Ions
      g4xs = new G4CrossSectionInelastic( new G4ComponentGGNuclNuclXsc );
   } 
   else if ( g4pd == G4AntiProton::Definition() ||
	     g4pd == G4AntiNeutron::Definition() ||
	     g4pd == G4AntiDeuteron::Definition() ||
	     g4pd == G4AntiTriton::Definition() ||
	     g4pd == G4AntiHe3::Definition() ||
	     g4pd == G4AntiAlpha::Definition() ) 
   {
      g4xs = new G4CrossSectionInelastic( new G4ComponentAntiNuclNuclearXS ); 
   } 
   else 
   {
      g4xs = new G4CrossSectionInelastic( new G4ComponentGGHadronNucleusXsc );
   }
   
   assert(g4xs);
      
   g4xs->BuildPhysicsTable( *g4pd );
   fXSecOnTarget = g4xs->GetCrossSection( &g4pdyn, elm );
...
bool G4VMP::G4VmpAnalyzerBase::ensureBeamTgtConfig( const art::Event& e )
{
...
void G4VMP::G4VmpAnalyzerBase::prepareG4PTable()
{
...
void G4VMP::G4VmpAnalyzerBase::Store4Professor( const std::string& label )
{
