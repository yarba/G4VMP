...
// --> no need ??? --> #include "art/Framework/Services/Registry/ServiceHandle.h"
...
G4VMP::EventGenerator::EventGenerator( fhicl::ParameterSet const& pset ):
  EDProducer(pset),
  fNParticles(pset.get<int>("nparticles",1)),
...
G4VMP::EventGenerator::~EventGenerator()
{
   if ( fRandom ) delete fRandom;
}

void G4VMP::EventGenerator::produce( art::Event& event ){

  // Create an empty output data product
  std::unique_ptr<GenParticleCollection> gens(new GenParticleCollection());
  gens->reserve(fNParticles);
...
DEFINE_ART_MODULE(G4VMP::EventGenerator)
