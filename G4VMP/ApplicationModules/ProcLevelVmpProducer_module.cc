...
#include "G4VMP/G4WrapperComponents/FTFPWrapper.hh"
// PreCo (also needed for FTF & QGS)
#include "Geant4/G4GeneratorPrecompoundInterface.hh"

// model setup & params change services 
#include "G4VMP/G4Services/PhysModelConfig_service.hh"
#include "G4VMP/G4Services/ProcLevelSimSetup_service.hh"

// Event/Run data products (produced here)
#include "G4VMP/DataProducts/Interactions/G4VmpVtx.hh" // Evt product
#include "G4VMP/DataProducts/ModelConfig/G4VmpModelConfig.hh" // Run product
...
namespace G4VMP {
  ...
  class ProcLevelVmpProducer : public art::EDProducer {

  public:

    explicit ProcLevelVmpProducer( const fhicl::ParameterSet& pset );
    virtual ~ProcLevelVmpProducer();
...
G4VMP::ProcLevelVmpProducer::ProcLevelVmpProducer( const fhicl::ParameterSet& p )
   :  art::EDProducer(p), 
      fLogInfo("ProcLevelVmpProducer")
{
...
*/
   ...
   fModelConfig = new G4VmpModelConfig(fDefaultPhysics);
   if ( !fDefaultPhysics )
   {
...
// !!!      G4cout << " Cross-check xsecScale = = " << G4CascadeParameters::xsecScale() << G4endl;
   ...
   //
   produces<G4VmpModelConfig,art::InRun>();
...
G4VMP::ProcLevelVmpProducer::~ProcLevelVmpProducer()
{
...
// At begin job
//
void G4VMP::ProcLevelVmpProducer::beginJob()
{
...
// At begin run
//
void G4VMP::ProcLevelVmpProducer::beginRun( art::Run& run )
{  
...
void G4VMP::ProcLevelVmpProducer::produce( art::Event& e )
{
   ...
   //
   std::unique_ptr<G4VmpVtx> firstint(new G4VmpVtx());
   ...
   //
   firstint->SetIncoming( G4VmpParticle( fTrack->GetParticleDefinition()->GetPDGEncoding(),
	                                 fTrack->GetMomentum() ) );

   for ( int is=0; is<nsec; ++is )
   {
      G4Track* strk = thechange->GetSecondary(is);
      firstint->AddOutcoming( G4VmpParticle( strk->GetParticleDefinition()->GetPDGEncoding(),
	                                     strk->GetMomentum() ) );
   }
...
// At end run
//
void G4VMP::ProcLevelVmpProducer::endRun( art::Run& )
{
...
void G4VMP::ProcLevelVmpProducer::initParticles()
{
...
void G4VMP::ProcLevelVmpProducer::initProcess()
{
...
void G4VMP::ProcLevelVmpProducer::setProjectile( const GenParticle& genpart )
{
   ...
   G4ParticleDefinition* partDef = 
      (G4ParticleTable::GetParticleTable())->FindParticle( genpart.pdgId() );

   // Track
   //
   fTrack = new G4Track( new G4DynamicParticle( partDef, 
                                                genpart.momentum()*CLHEP::GeV ), // momentum in GeV/c !!! 
...
void G4VMP::ProcLevelVmpProducer::resetProjectileKine( const GenParticle& part )
{
...
DEFINE_ART_MODULE(G4VMP::ProcLevelVmpProducer)
